...
import type { ScanResult, PhotoDetail, PhotoQuery, TagExportItem, TagImportResult } from "@/types";
...
/**
 * Returns up to `count` photos in a single request, optionally scoped to a search query,
 * a folder, and optionally shuffled.
 * Returns an empty array when no photo matches the requested scope (HTTP 204).
 *
 * Scoping precedence mirrors the backend: when `query` is supplied the selection is scoped to
 * the matching search results; otherwise `folderId`, if supplied, scopes it to that folder's
 * subtree; otherwise the whole library is used. The query is sent as a single JSON parameter so
 * it can carry additional filter fields in future without changing this signature.
 *
 ...
 * @param query     search criteria to scope the selection to, or null for no search scope
...
export const getPhotos = async (
    ...
    query: PhotoQuery | null = null
): Promise<import("@/types").MediaFile[]> => {
    const response = await axios.get(config.defaultServer() + '/api/photos/batch', {
        params: { count, folderId, doShuffle, afterId, query: query ? JSON.stringify(query) : null },
