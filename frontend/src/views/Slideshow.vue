...
const doShuffle = ref(true)
/** Bumped whenever the ordering mode changes, so fetches issued under the old mode are discarded on resolve. */
let fetchEpoch = 0
...
/**
 * Toggles between random and sequential photo ordering.
 *
 * Discards every photo queued under the previous mode: it bumps the fetch epoch so
 * any in-flight fetch is ignored when it resolves, clears the prefetch queue, and
 * drops any forward history. This guarantees the next photos come fresh from the
 * server under the new ordering mode rather than replaying stale entries. Finally it
 * warms a new batch continuing from the current photo.
 */
const toggleShuffle = () => {
	doShuffle.value = !doShuffle.value
	fetchEpoch++
	for (const photo of prefetchQueue.value) releaseImage(photo.id)
	prefetchQueue.value = []
	if (historyIndex.value < history.value.length - 1) {
		for (const photo of history.value.slice(historyIndex.value + 1)) releaseImage(photo.id)
...
/**
 * Returns the id to resume a sequential fetch from: the last photo currently known
 * going forward (tail of the prefetch queue, or the current photo). Returns null in
 * shuffle mode (random fetches need no cursor) or when nothing is loaded yet.
 */
const forwardCursor = (): number | null => {
	if (doShuffle.value) return null
	if (prefetchQueue.value.length > 0) return prefetchQueue.value[prefetchQueue.value.length - 1].id
	return currentPhoto.value?.id ?? null
}
	...
	<div class="slideshow" ref="slideshowEl">
		<template v-if="currentPhoto">
			...
			<div class="slideshow__hud-right">
				...
				<button
					class="slideshow__shuffle-btn"
					:class="{ 'slideshow__shuffle-btn--active': doShuffle }"
					@click="toggleShuffle"
					:title="doShuffle ? language.get('Shuffle: on') : language.get('Shuffle: off')"
				>
					<svg v-if="!doShuffle" xmlns="http://www.w3.org/2000/svg" width="20" fill="currentColor" class="bi bi-shuffle" viewBox="0 0 16 16">
					...
					<svg v-else xmlns="http://www.w3.org/2000/svg" width="20" fill="currentColor" class="bi bi-list-ol" viewBox="0 0 16 16">
